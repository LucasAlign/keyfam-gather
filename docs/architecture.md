...
The application ships as a self-contained Next.js standalone build (`output: "standalone"`). The multi-stage `Dockerfile` installs dependencies from the lockfile, generates the Prisma client, and builds the standalone server. The `runner` target copies only the standalone output and static assets into a minimal `node:22-alpine` image that runs `node server.js` as a non-root user; the Prisma query engine is traced into the standalone output, so the runtime image needs no extra Prisma files. Migrations are applied out of band — by `npm run db:deploy` as a release/pre-deploy step where the source tree is available, or by the dedicated `migrator` image target (`docker build --target migrator`), which retains the full dependency tree and runs `prisma migrate deploy`. App replicas never migrate, so they cannot race each other, and the container never runs `migrate dev`.
...
Two unauthenticated probes support orchestrated deployments. `GET /healthz` is a **liveness** probe that returns `200 {"status":"ok"}` while touching no dependency, so a healthy instance is never restarted merely because Postgres is briefly unreachable. `GET /readyz` is a **readiness** probe that runs `SELECT 1`; it returns `200` with the round-trip `latencyMs` when the database answers and `503` otherwise, so a load balancer drains an instance that has lost its database and re-adds it on recovery without a restart. Both send `Cache-Control: no-store` and are pinned `dynamic = "force-dynamic"`.

Logging is dependency-free and structured: `src/lib/logger.ts` emits one JSON line per event — `info`/`debug` to stdout, `warn`/`error` to stderr — filtered by `LOG_LEVEL` (default `info`), so a log shipper parses fields without regex. `serializeError` flattens throwables and, critically, preserves the Prisma error code so a transient conflict (`P2034`) stays distinguishable from a real fault in the logs. The attendance sync route logs per-batch throughput and the disposition mix (never attendee data), which reveals a device retrying against conflicts during a busy check-in.

Server errors are captured centrally through Next's `onRequestError` hook in `src/instrumentation.ts` — route handlers, server components, and server actions alike — rather than by wrapping each entry point. Each captured error is logged as a structured record (path, method, route type, digest); if `ERROR_WEBHOOK_URL` is set, the record is also POSTed there for an external monitor, with forwarding failures swallowed after logging so they never mask the original error.
...
QR check-in adds `CheckInToken`, a per-registration bearer credential that mirrors the host-access-token pattern exactly: an opaque 256-bit value stored only as a SHA-256 digest, optional expiry, revocation, and throttled `lastUsedAt` tracking. Issuing or reissuing a token reveals the raw value once, through the same query-param reveal used for host and invitation links; it is never persisted or logged. A registration may have at most one active token — reissuing revokes the prior one in the same transaction, so a lost badge can be invalidated without a gap. `checkin:manage` gates issuance, reissue, revoke, and resolution.

Scanning never creates a parallel check-in path. `/events/[eventId]/check-in/qr` is a rate-limited route that authorizes the operator (`checkin:manage`, event-scoped) before resolving the token digest to a registration ID, re-checking organization, event, and active-registration scope; it returns only the registration ID, already visible to the authorized operator elsewhere in the workspace. The check-in workspace's QR panel then drives the identical `AttendanceCommand`/offline-queue/sync path as manual check-in, always issuing `CHECK_IN` (never `UNDO`) so a repeat scan is idempotent through the existing `ALREADY_CHECKED_IN` conflict handling rather than a client-side toggle. Camera scanning uses the browser `BarcodeDetector` API where available (feature-detected, no bundled decoding library); a scanner-gun/typed-code text input always works as the baseline path. QR payloads encode only the opaque token, never a URL or database ID.

Invitation delivery introduces a provider-abstracted seam (`src/lib/delivery`) with a `DeliveryProvider` interface, one `DeliveryMessage`/`DeliveryResult` contract, and per-channel provider registries selected by `EMAIL_DELIVERY_PROVIDER`/`SMS_DELIVERY_PROVIDER` (defaulting to `log`, since no provider credentials exist in this environment). The default log provider makes no network call and logs only redacted recipient metadata — never a message body, which may embed a secure single-reveal invitation link. `recordInvitationDelivery` (`src/lib/invitation-delivery.ts`) resolves email-first/SMS-fallback channel selection, builds the message, calls the provider, and records a `DeliveryAttempt` plus an audit entry in one transaction; it is a plain function callable outside a request context (used directly by the PostgreSQL verification script), independent of the "use server" action that triggers it. Staff Send/Resend and host Invite/Resend emit delivery after their token-issuing transaction commits; a delivery-recording failure is logged and never blocks or rolls back the invitation state change, preserving the existing secure-link model where the sender can always copy the link regardless of delivery outcome.

## Shared invitation core

The invitation lifecycle has one implementation, `src/lib/invitation-core.ts`, exposing a function per operation (`sendInvitation`, `createInvitationDraft`, `setInvitationStaffStatus`, `createHostInvitation`, `resendHostInvitation`, `cancelHostInvitation`, `registerFromInvitation`, `declineInvitation`, and the idempotent `markInvitationOpened` side-effect). The core owns every transaction, status guard, audit write, and — for the send/resend paths — delivery, and raises a transport-neutral `InvitationError` carrying an HTTP status and optional field errors. Two thin adapters translate I/O only: the in-app server actions (`src/app/invitation-actions.ts`, FormData in → `revalidatePath`/`redirect` or form state out, plus request-scoped rate limiting) and the Align Core HTTP API (`src/lib/invitation-service.ts`, JSON in → serialized envelope out, with its `ApiError` extending `InvitationError` so a route maps either identically). Because delivery is emitted from the core, both surfaces — and the invitee page's open side-effect — behave identically by construction; the HTTP send/host paths that previously skipped delivery now record a `DeliveryAttempt`, asserted by `verify-postgres`.
