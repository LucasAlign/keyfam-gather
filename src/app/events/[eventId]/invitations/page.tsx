...
export default async function InvitationsPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams: Promise<{ created?: string; sent?: string; token?: string }> }) {
  ...
  const event = await db.event.findUnique({ where: { id: eventId }, include: { groups: { orderBy: { name: "asc" } }, invitations: { include: { group: true, registration: true, deliveryAttempts: { orderBy: { createdAt: "desc" }, take: 1 } }, orderBy: { createdAt: "desc" } } } });
  if (!event) notFound();
  ...
  return <>
    ...
    <div className="invitation-layout"><section><div className="section-heading"><h2>Invitation status</h2><span>{event.invitations.length}</span></div>
      {event.invitations.length === 0 ? <div className="empty compact"><h3>No invitations yet</h3><p>Create the first invitation draft.</p></div> : <div className="invitation-list">{event.invitations.map((invitation) => {
        ...
        const delivery = invitation.deliveryAttempts[0];
        const deliveryLabel = delivery ? `${delivery.channel === "EMAIL" ? "Email" : "SMS"} ${delivery.status === "SENT" ? "delivered" : "delivery failed"}` : null;
        return <article key={invitation.id}><div><strong>{invitation.firstName} {invitation.lastName}</strong><p>{invitation.email ?? invitation.phone}{invitation.group ? ` · ${invitation.group.name}` : ""}</p><small>{invitation.sentAt ? `Sent ${invitation.sentAt.toLocaleDateString()}` : "Not sent yet"}{deliveryLabel ? ` · ${deliveryLabel}` : ""}</small></div><span className={`invitation-status status-${invitation.status.toLowerCase()}`}>{invitationStatusLabel(invitation.status)}</span>{actionable && <div className="invitation-actions"><form action={sendInvitation}><input type="hidden" name="eventId" value={eventId} /><input type="hidden" name="invitationId" value={invitation.id} /><button type="submit">{invitation.sentAt ? "Resend" : "Send"}</button></form>{invitation.sentAt && <form action={markInvitationNoResponse}><input type="hidden" name="eventId" value={eventId} /><input type="hidden" name="invitationId" value={invitation.id} /><button type="submit">No response</button></form>}<form action={cancelInvitation}><input type="hidden" name="eventId" value={eventId} /><input type="hidden" name="invitationId" value={invitation.id} /><button type="submit">Cancel</button></form></div>}</article>;
      })}</div>}
