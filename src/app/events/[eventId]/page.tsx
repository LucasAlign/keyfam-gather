...
export default async function EventPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams: Promise<{ registered?: string }> }) {
  ...
  return <><Link className="back" href="/">← Events</Link>{registered && <div className="success" role="status">Registrant added successfully.</div>}<div className="page-heading"><div><span className="status">{event.status.replaceAll("_", " ")}</span><h1>{event.name}</h1><p>{new Intl.DateTimeFormat("en-US", { dateStyle: "full", timeStyle: "short", timeZone: event.timezone }).format(event.startsAt)}{event.venue ? ` · ${event.venue}` : ""}</p></div><div className="button-row"><Link className="button" href={`/events/${event.id}/check-in`}>Open check-in</Link><Link className="button secondary" href={`/events/${event.id}/seating`}>Tables & seating</Link><Link className="button secondary" href={`/events/${event.id}/hosts/new`}>Hosts & groups</Link><Link className="button" href={`/events/${event.id}/register`}>Add registrant</Link></div></div>
