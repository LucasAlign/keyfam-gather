...
export default async function CheckInPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!event) notFound();
  const { membership } = await requireActor(event.organizationId, "checkin:manage");
  ...
  const registrants = event.registrations.map((item) => ({ id: item.id, name: `${item.person.firstName} ${item.person.lastName}`, email: item.person.email, phone: item.person.phone, group: item.group?.name ?? null, table: item.table?.name ?? null, party: item.party?.name ?? null, checkIn: item.checkIn?.reversedAt === null ? { checkedInAt: item.checkIn.checkedInAt.toISOString(), actor: item.checkIn.actor.name, deviceId: item.checkIn.deviceId } : null }));
