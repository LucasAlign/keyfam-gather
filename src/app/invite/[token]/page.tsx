...
export default async function InvitationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const invitation = await db.invitation.findUnique({ where: { tokenHash: hashInvitationToken(token) }, include: { event: { include: { registrationFields: { where: { isActive: true, visibility: "PUBLIC" }, include: { options: { orderBy: { sortOrder: "asc" } } }, orderBy: { sortOrder: "asc" } } } }, group: true } });
  ...
  return <div className="narrow"><div className="portal-hero"><p className="eyebrow">You’re invited</p><h1>{invitation.event.name}</h1><p>{new Intl.DateTimeFormat("en-US", { dateStyle: "full", timeStyle: "short", timeZone: invitation.event.timezone }).format(invitation.event.startsAt)}{invitation.event.venue ? ` · ${invitation.event.venue}` : ""}</p>{invitation.group && <p>Hosted with {invitation.group.name}</p>}</div>
    <section><h2>Register for the event</h2><p>Confirm or update your details below.</p>{canRespond && <InvitationRegistrationForm token={token} invitation={invitation} fields={invitation.event.registrationFields} />}</section>
