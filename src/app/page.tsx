...
export default async function Home() {
  ...
  const events = await db.event.findMany({ where: { organizationId: organization.id, ...(canViewEveryEvent ? {} : { id: { in: access.eventAssignments.map(({ eventId }) => eventId) } }) }, orderBy: { startsAt: "asc" }, include: { _count: { select: { registrations: true } } } });
