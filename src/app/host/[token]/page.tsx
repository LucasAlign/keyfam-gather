...
export default async function HostPortalPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ registered?: string; invited?: string; inviteToken?: string }> }) {
  ...
  const activeRegistrations = eventHost.group.registrations.filter((registration) => registration.status === "ACTIVE");
  const occupied = activeRegistrations.length;
  const remaining = remainingSeats(eventHost.group.capacity, occupied);
  const guests = activeRegistrations.filter((registration) => registration.personId !== eventHost.personId);
  const cancelledGuests = eventHost.group.registrations.filter((registration) => registration.personId !== eventHost.personId && registration.status === "CANCELLED");
  ...
  return <>
    ...
    <div className="portal-grid"><section><div className="section-heading"><h2>Your guests</h2><span>{guests.length}</span></div>{guests.length === 0 ? <div className="empty compact"><h3>No registered guests yet</h3><p>Add a guest directly or send an invitation.</p></div> : <div className="registrants">{guests.map(({ id, person, status }) => <article key={id}><div className="avatar">{person.firstName[0]}{person.lastName[0]}</div><div><strong>{person.firstName} {person.lastName}</strong><p>{person.email ?? person.phone}</p><RegistrationEditor registration={{ id, status, firstName: person.firstName, lastName: person.lastName, email: person.email, phone: person.phone }} context={{ kind: "host", token }} /></div><span>Registered</span></article>)}</div>}
      {cancelledGuests.length > 0 && <><div className="section-heading invitation-heading"><h2>Cancelled guests</h2><span>{cancelledGuests.length}</span></div><div className="registrants">{cancelledGuests.map(({ id, person, status }) => <article className="is-cancelled" key={id}><div className="avatar">{person.firstName[0]}{person.lastName[0]}</div><div><strong>{person.firstName} {person.lastName}</strong><p>{person.email ?? person.phone}</p><RegistrationEditor registration={{ id, status, firstName: person.firstName, lastName: person.lastName, email: person.email, phone: person.phone }} context={{ kind: "host", token }} /></div><span>Cancelled</span></article>)}</div></>}
