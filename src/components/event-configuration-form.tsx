...
export function EventConfigurationForm({ event, nextStatus, canDuplicate }: { event: Event; nextStatus: EventStatus | null; canDuplicate: boolean }) {
  ...
  return <div className="configuration-layout">
    <form action={updateAction} className="form-card configuration-form">
      ...
      <div className="field-row"><label>Event name<input name="name" defaultValue={event.name} disabled={archived} required />{error("name") && <small>{error("name")}</small>}</label><label>Event type<input name="eventType" defaultValue={event.eventType} placeholder="Fundraising banquet, gala, volunteer dinner" disabled={archived} required /><small>Use the familiar name your team uses for this kind of Event.</small></label></div>
