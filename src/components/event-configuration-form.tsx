...
export function EventConfigurationForm({ event, nextStatus, canDuplicate }: { event: Event; nextStatus: EventStatus | null; canDuplicate: boolean }) {
  ...
  return <div className="configuration-layout">
    <form key={`${event.updatedAt.toISOString()}:${updateState.token ?? "initial"}`} action={updateAction} className="form-card configuration-form">
      ...
      <p className="form-hint">Times below use {event.timezone}. Currently {new Intl.DateTimeFormat("en-US", { dateStyle: "full", timeStyle: "short", timeZone: event.timezone }).format(event.startsAt)} through {new Intl.DateTimeFormat("en-US", { dateStyle: "full", timeStyle: "short", timeZone: event.timezone }).format(event.endsAt)}.</p><EventScheduleFields defaultStart={keep("startsAt", eventLocalDateTime(event.startsAt, event.timezone))} defaultEnd={keep("endsAt", eventLocalDateTime(event.endsAt, event.timezone))} disabled={archived} endError={error("endsAt")} />
      <div className="field-row"><label>Timezone<input name="timezone" defaultValue={keep("timezone", event.timezone)} placeholder="America/New_York" disabled={archived} required /><small>IANA timezone, e.g. America/New_York or America/Chicago. All event times above are shown in it.</small></label><label>Capacity<input name="capacity" type="number" min="1" defaultValue={keep("capacity", event.capacity === null ? "" : String(event.capacity))} disabled={archived} /></label></div>
