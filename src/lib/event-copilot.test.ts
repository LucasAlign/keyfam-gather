...
import { allowsOperationalRecommendations, detectReadinessRecommendations } from "./event-copilot";

describe("Event Copilot readiness detector", () => {
  it("stops pre-Event recommendations after completion", () => { expect(allowsOperationalRecommendations("EVENT_LIVE")).toBe(true); expect(allowsOperationalRecommendations("COMPLETED")).toBe(false); expect(allowsOperationalRecommendations("ARCHIVED")).toBe(false); });
