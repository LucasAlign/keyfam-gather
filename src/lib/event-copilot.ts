...
async function currentProposals(eventId: string, now = new Date()) {
  ...
  const forecast = forecastRegistrationPace({ goal: event.capacity, registered: event._count.registrations, daysRemaining, recentVelocityPerDay: event.registrations.filter((item) => item.registeredAt >= recentCutoff).length / 7, velocityWindowDays: 7, calculatedAt: now });
