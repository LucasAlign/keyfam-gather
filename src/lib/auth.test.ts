...
describe("Vertical 2 authorization boundaries", () => {
  it("allows admins, but not event staff or viewers, to manage hosts", () => {
    ...
    expect(roleHasCapability(MembershipRole.EVENT_ADMIN, "host:manage")).toBe(true);
    expect(roleHasCapability(MembershipRole.EVENT_STAFF, "host:manage")).toBe(false);
...
describe("Vertical 3 authorization boundaries", () => {
  it("allows event staff to manage seating but keeps viewers read-only", () => {
    expect(roleHasCapability(MembershipRole.EVENT_STAFF, "seating:manage")).toBe(true);
...
describe("Vertical 4 authorization boundaries", () => {
  it("allows event staff to operate check-in but keeps viewers read-only", () => {
    ...
    expect(roleHasCapability(MembershipRole.EVENT_ADMIN, "checkin:manage")).toBe(true);
    expect(roleHasCapability(MembershipRole.EVENT_STAFF, "checkin:manage")).toBe(true);
...
describe("Vertical 5 authorization boundaries", () => {
  it("keeps walk-in exception controls away from basic event staff", () => {
    ...
    expect(roleHasCapability(MembershipRole.EVENT_ADMIN, "walkin:manage")).toBe(true);
    expect(roleHasCapability(MembershipRole.EVENT_STAFF, "walkin:manage")).toBe(false);
