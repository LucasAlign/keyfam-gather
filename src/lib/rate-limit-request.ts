...
import { enforcePublicRateLimit, PublicRateLimitError } from "@/lib/public-rate-limit";
...
export async function enforceIpRateLimit(scope: string, limit: number, windowMs: number): Promise<RateLimitResult> {
  ...
  try {
    await enforcePublicRateLimit(`ip:${scope}`, clientIpFromHeaders(source), { limit, windowSeconds: Math.max(1, Math.ceil(windowMs / 1000)) });
    return { allowed: true, retryAfterSeconds: 0 };
  } catch (error) {
    if (error instanceof PublicRateLimitError) return { allowed: false, retryAfterSeconds: error.retryAfterSeconds };
    throw error;
  }
}
